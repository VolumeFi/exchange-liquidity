...
#  @notice This Vyper contract removes liquidity to any Sushiswap pool using ETH or any ERC20 Token.
...
@payable
@nonreentrant('lock')
def divestSushiPairToToken(pair: address, token: address, amount: uint256, amountMin: uint256, deadline: uint256=MAX_UINT256) -> uint256:
    ...
    assert pair != ZERO_ADDRESS, "Invalid Sushipool Address"
    ...
    ERC20(pair).approve(SUSHISWAPROUTER, amount)

    token0Amount: uint256 = 0
    token1Amount: uint256 = 0
    (token0Amount, token1Amount) = SushiswapRouter(SUSHISWAPROUTER).removeLiquidity(token0, token1, amount, 1, 1, self, deadline)
    ...
    if token == token0:
        ...
        assert tokenAmount >= amountMin, "High Slippage"
        return tokenAmount
    elif token == token1:
...
def removeLiquidity(tokenA: address, tokenB: address, liquidity: uint256, amountAMin: uint256, amountBMin: uint256, to: address, deadline: uint256=DEADLINE) -> (uint256, uint256):
    ...
    amount0: uint256 = 0
    amount1: uint256 = 0
    (amount0, amount1) = SushiswapPair(pair).burn(to)
...
def removeLiquidityETH(token: address, liquidity: uint256, amountTokenMin: uint256, amountETHMin: uint256, to: address, deadline: uint256=DEADLINE) -> (uint256, uint256):
    assert block.timestamp <= deadline, "Expired!"
    pair: address = SushiswapFactory(SUSHISWAPFACTORY).getPair(token, WETH)
    ERC20(pair).transferFrom(msg.sender, pair, liquidity)
    amount0: uint256 = 0
    amount1: uint256 = 0
    (amount0, amount1) = SushiswapPair(pair).burn(self)
